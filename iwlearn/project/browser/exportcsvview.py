...
class ExportCSVView(BrowserView):
    ...
    def __call__(self):
        ...
        for brain in brains:
            ...
            pd['Country']= '; '.join(obj.getField('country').get(obj))
            pd['Type']= obj.getProject_type()
            pd['Project Type'] = "; ".join(obj.getProject_category())
            pd['Project Scale']= obj.getProject_scale()
            pd['Ecosystem']= "; ".join(obj.getEcosystem())
            pd['Region']= obj.getField('region').get(obj)
            pd['Subregion']= obj.getField('subregion').get(obj)
            pd['Basin']= '; '.join(obj.getField('basin').get(obj))
