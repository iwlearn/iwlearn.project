...
class ImportRACSV(ImportCSV):
    ...
    def update_project(self, project, data):
            ...
            project.setImcs_desc(data['Establishment of country-specific inter-ministerial committees'])
            project.setImcs(data['Rating: Establishment of country-specific inter-ministerial committees'].strip('n/a'))
            project.setRegional_frameworks_desc(data['Regional legal agreements and cooperation frameworks'])
            project.setRegional_frameworks(data['Rating: Regional legal agreements and cooperation frameworks'].strip('n/a'))
            project.setRmis_desc(data['Regional Management Institutions'])
            project.setRmis(data['Rating: Regional Management Institutions'].strip('n/a'))
            project.setReforms_desc(data['National/Local reforms'])
            project.setReforms(data['Rating: National/Local reforms'].strip('n/a'))
            ...
            project.setTda_priorities(data['Rating: Transboundary Diagnostic Analysis: Agreement on transboundary priorities and root causes'].strip('n/a'))
            project.setSap_devel_desc(data['Development of Strategic Action Plan (SAP)'])
            project.setSap_devel(data['Rating: Development of Strategic Action Plan (SAP)'].strip('n/a'))
            project.setAbnj_rmi_desc(data['Management measures in ABNJ incorporated in  Global/Regional Management Organizations (RMI)'])
            project.setAbnj_rmi(data['Rating: Management measures in ABNJ incorporated in  Global/Regional Management Organizations (RMI)'].strip('n/a'))
            ...
            project.setTdasap_cc(data['Rating: Revised Transboundary Diagnostic Analysis (TDA)/Strategic Action Program (SAP) including Climatic Variability and Change considerations'].strip('n/a'))
            project.setTda_mnits_desc(data['TDA based on multi-national, interdisciplinary technical and scientific (MNITS) activities'])
            project.setTda_mnits(data['Rating: TDA based on multi-national, interdisciplinary technical and scientific (MNITS) activities'].strip('n/a'))
            project.setSap_adopted_desc(data['Proportion of Countries that have adopted SAP'])
            try:
                i = data['Rating: Proportion of Countries that have adopted SAP'].strip('%').strip('n/a')
                if i:
                    sap_adopted= int(float(i))
                    project.setSap_adopted(sap_adopted)
            except:
                logger.error('project %s has invalid data in: Proportion of Countries that have adopted SAP' % data[self.id_column] )
            ...
            try:
                i = data['Rating: Proportion of countries that are implementing specific measures from the SAP (i.e. adopted national policies, laws, budgeted plans)'].strip('%').strip('n/a')
                if i:
                    sap_implementing= int(float(i))
                    project.setSap_implementing(sap_implementing)
            except:
                logger.error('project %s has invalid data in: Proportion of countries that are implementing specific measures from the SAP' % data[self.id_column] )
            ...
            project.setSap_inc(data['Rating: Incorporation of (SAP, etc.) priorities with clear commitments and time frames into CAS, PRSPs, UN Frameworks, UNDAF, key agency strategic documents including financial commitments and time frames, etc'].strip('n/a'))
